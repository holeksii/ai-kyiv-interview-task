...
object CsvUtils {
  ...
  /** Reads records from a CSV file.
    *
    * Examples: val record: CSVRecord = ... record.size() // the number of
    * columns in this record record.get(2) // returns the third column as a
    * String
    */
  ...
  /** Writes records to a file in a CSV format.
    *
    * Each record is a List[Any], e.g. this snippet writes two records to a
    * file: val records = List( List("John", 41, "Plumber"), List("Misato-san",
    * 29, "Operations Director") ) CsvUtils.writeToFile(records, new
    * File("employees.csv"))
    */
